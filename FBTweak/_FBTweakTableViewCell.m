...
- (void)layoutSubviews
{
  if (_mode == _FBTweakTableViewCellModeBoolean) {
    ...
    CGRect textFrame = CGRectMake(0, 0, self.bounds.size.width / 2, self.bounds.size.height);
    _textField.frame = CGRectIntegral(textFrame);
    _accessoryView.bounds = CGRectIntegral(textFrame);
...
- (void)_updateMode:(_FBTweakTableViewCellMode)mode
{
  ...
  if (_mode == _FBTweakTableViewCellModeBoolean) {
    ...
    if (_tweak.stepValue) {
      _stepper.stepValue = [_tweak.stepValue doubleValue];
    } else if (_stepper.maximumValue != _stepper.minimumValue) {
